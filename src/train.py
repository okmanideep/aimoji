...
def embed_texts(model: SentenceTransformer, texts: Sequence[str], batch_size: int = 256) -> np.ndarray:
    return model.encode(
        list(texts),
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Train MiniLM + Logistic Regression emoji predictor")
    ap.add_argument("--data-dir", default="data/processed", help="Parquet dir with train/eval/test")
    ap.add_argument("--outdir", default="models/minilm_lr", help="Where to save classifier artifacts")
    ap.add_argument("--batch-size", type=int, default=256)
    ap.add_argument("--max-examples", type=int, default=0, help="Subset for quick runs (0 = all)")
    ...
    joblib.dump({"classifier": clf, "label_encoder": le, "model_name": MODEL_NAME}, out_path)
